...
```{r setup, fig.width=8, fig.height=6, fig.cap="All tweets from our accounts"}
library(lubridate)
library(ggplot2)
library(dplyr)
library(readr)
...
```{r top_dave, dependson = c("words_by_time", "top_slopes2"), fig.width=8, fig.height=5, fig.cap = "Trending words in David's tweets"}
words_by_time %>%
...
```{r top_julia, dependson = c("words_by_time", "top_slopes2"), fig.width=8, fig.height=5, fig.cap="Trending words in Julia's tweets"}
words_by_time %>%
  inner_join(top_slopes, by = c("word", "person")) %>%
  filter(person == "Julia") %>%
  ggplot(aes(time_floor, count/time_total, color = word)) +
...
```{r plot_rts, dependson = "word_by_rts", fig.width=10, fig.height=5, fig.cap="Words with highest median retweets"}
word_by_rts %>%
...
```{r plot_favs, dependson = "word_by_favs", fig.width=10, fig.height=5, fig.cap="Words with highest median favorites"}
word_by_favs %>%
