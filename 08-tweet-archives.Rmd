...
We are about equally likely to tweet about programming, writing, and cool things like the Broadway musical *Hamilton*.
...
So David has tweeted about matrices, models, and his R packages like broom and gganimate while Julia tweeted about Salt Lake City, physics, and maps.
...
```{r nested_models, dependson = "nest", eval = FALSE}
library(purrr)
...
```{r slopes, dependson = "nested_models", eval = FALSE}
library(broom)
...
```{r top_slopes2, dependson = "slopes", eval = FALSE}
top_slopes <- slopes %>% 
...
```{r dependson = c("words_by_time", "top_slopes2"), fig.width=5, fig.height=8, eval = FALSE}
words_by_time %>%
...
At the top of this sorted data frame, we see David's tweet about his blog post on Donald Trump's own tweets. A search tells us that this is the only time David has ever used the word "angrier" in his tweets, so that word has an extremely high median retweet rate.
