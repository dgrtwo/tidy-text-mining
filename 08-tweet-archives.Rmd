...
$$\text{log odds} = \ln{\left(\frac{\left(\frac{n+1}{\text{total}+1}\right)_\text{David}}{\left(\frac{n+1}{\text{total}+1}\right)_\text{Julia}}\right)}$$
where $n$ is the number of times the word in question is used by each person and the total means the total words for each person.
...
```{r plotratios, dependson = "word_ratios", fig.width=6, fig.height=6}
word_ratios %>%
  group_by(logratio < 0) %>%
