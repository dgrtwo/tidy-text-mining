...
```{r negated_words_plot, dependson = "negated_words", fig.width=9, fig.height=9, echo = FALSE}
negated_words %>%
  mutate(contribution = n * score) %>%
...
```{r bigram_ggraph_austen, dependson = "bigram_graph", fig.width = 10, fig.height = 10}
library(ggraph)
set.seed(2016)
...
```{r bigram_ggraph_austen2, dependson = "bigram_graph", fig.width = 10, fig.height = 10}
set.seed(2016)
...
```{r kjv, echo = FALSE}
load("~/Repositories/tidy-text-mining/data/kjv.rda")
```
...
![The philosophy behind the widyr package, which can operations such as counting and correlating on pairs of values in a tidy dataset.](/Users/dgrtwo/Repositories/tidy-text-mining/images/widyr.jpg)
...
```{r word_cors_network, dependson = "word_cors", fig.height = 8, fig.width = 8}
set.seed(2016)
