...
```{r bigramtfidf, dependson = "bigram_tf_idf", echo = FALSE, fig.width=9, fig.height=9, fig.cap = "The 12 bigrams with the highest TF-IDF from each Jane Austen novel."}
bigram_tf_idf %>%
  arrange(desc(tf_idf)) %>%
  top_n(12, tf_idf) %>%
  ungroup() %>%
...
```{r bigramgraph, dependson = "bigram_graph", fig.width = 10, fig.height = 10, fig.cap = "Common bigrams in Pride and Prejudice, showing those that occurred more than 20 times and where neither word was a stop-word."}
library(ggraph)
set.seed(2017)
...
```{r bigramggraphausten2, dependson = "bigram_graph", fig.width = 10, fig.height = 10, fig.cap = "Common bigrams in Pride and Prejudice, with some polishing."}
set.seed(2016)
...
```{r kjvbigrams, dependson = "kjv", fig.cap = "Directed graph of common bigrams in the King James Bible, showing those that occurred more than 40 times."}
library(stringr)
...
```{r widyr, fig.cap = "The philosophy behind the widyr package, which can perform operations such as counting and correlating on pairs of values in a tidy dataset. The widyr package first 'casts' a tidy dataset into a wide matrix, performs an operation such as a correlation on it, then re-tidies the result."}
...
```{r wordcors, dependson = "word_cors", fig.height = 8, fig.width = 8, fig.cap = "Words from Pride and Prejudice that were most correlated with 'elizabeth', 'pounds', 'married', and 'pride'."}
word_cors %>%
...
```{r wordcorsnetwork, dependson = "word_cors", fig.height = 8, fig.width = 8, fig.cap = "Pairs of words in Pride and Prejudice that show at least a .15 correlation of appearing within the same 10-line section."}
set.seed(2016)
