...
```{r nrc_joy, dependson = "tidy_books"}
...
We see many positive, happy words about hope, friendship, and love here. We also see some hints of the caveats necessary with using a general-purpose sentiment dictionary -- `"present"` makes an appearance, but it had slightly different usage in Austen's era and was more often neutral than joyous in her writing (we can see this by reading in whole some of the lines where `present` appears). 

Alternatively, we could examine how sentiment changes throughout each novel. We can do this with just a handful of lines that are mostly dplyr functions. First, we find a sentiment score for each word using the Bing lexicon and `inner_join()`. 
...
```{r jane_austen_sentiment, dependson = "tidy_books"}
library(tidyr)
...
```{r sentimentplot, dependson = "jane_austen_sentiment", fig.width=9, fig.height=10, fig.cap="Sentiment through the narratives of Jane Austen's novels"}
library(ggplot2)
